 ...
 * 원래 이 클라이언트는 <b>무결과에 {@code null}, 호출 실패에 예외</b>를 냈다. 한 번 부르고 마는
 * 1-2의 실존 검증에는 충분했지만, 4-8의 지오코딩 캐스케이드는 이 둘을 <b>정반대로</b> 다뤄야 한다.
 *
 * <pre>
 ...
 *
 * <p>그 옛 계약은 {@code findBestPlace}라는 얇은 껍질로 한동안 함께 살아 있었으나, 8-1이 AI 코스
 * 경로를 파이프라인 호출로 교체하면서 호출부가 사라져 지웠다. <b>지금은 이 타입이 유일한 계약이다.</b>
 *
