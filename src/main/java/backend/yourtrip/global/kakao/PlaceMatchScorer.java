 ...
 * <b>소문자화만 하고 그대로 비교한다.</b> 두 규칙을 통일하고 싶어지지만 신중해야 한다 —
 * 환각률 하네스가 게이트 통과 후보의 <b>층화 추출 구간을 이 점수로 나누므로</b>, 바꾸면 측정 간
 * 구간 기록이 어긋난다. 게이트 통과 후에도 낮은 점수가 남는 것("동궁과 월지" → "동궁과월지"는
 * 게이트는 통과하지만 이름 +5는 못 받는다)은 알려진 성질이고, 그래서 이 점수는 정확도 신호가
 * 아니라 순위·층화의 축으로만 쓴다.
 */
