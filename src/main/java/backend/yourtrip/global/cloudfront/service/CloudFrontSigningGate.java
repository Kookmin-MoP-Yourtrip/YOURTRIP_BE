 ...
 * 잡히지 않는다. 이 게이트는 세마포어로 부하 차단을 요청 단위로 옮겨, 통과한 요청은
 * 완전한 응답을 받고 나머지는 명확한 503을 받게 한다.
 *
 * <p><b>서명이 코스당 1회가 된 뒤의 위상(1단계)</b>: 요청 하나가 이미지 수만큼 태스크를
 * fan-out하던 구조가 사라져, 이 게이트가 막으려던 "부분 응답"은 구조적으로 발생할 수 없게
 * 됐다(서명이 1건이라 전부 성공하거나 요청 전체가 실패한다). 그럼에도 이 클래스를 당장
 * 지우지 않는 이유는 "서명 횟수 감소"와 "격리 인프라 제거"의 효과를 분리 측정하기
 * 위해서다 — Run D에서 거부 카운터가 0으로 수렴하는 것을 확인한 뒤 제거한다
...
public class CloudFrontSigningGate {
    ...
    public CloudFrontSigningGate(
        ...
        // permits는 처리량이 아니라 지연 예산을 정한다(처리량 상한은 풀 크기만의 함수라
        // permits가 소거된다). 산정식은 application.yml의 cloudfront.signing.permits 참고.
        ...
        this.submitRejectedCounter = Counter.builder("cloudfront.signing.gate.submit.rejected")
            .description("게이트를 통과했지만 executor 제출 단계에서 거부된 요청 수 — "
    ...
    /**
     * 코스 스코프 서명 1건을 발급한다. 실패하면 <b>예외를 던진다</b>(fail-closed).
     *
     * <p>서명이 이미지 수만큼이던 시절에는 실패한 key만 빼고 나머지를 반환하는 fail-open이
     * 합리적이었다(사진 몇 장이 빠져도 일정은 보인다). 서명이 1건이 된 지금 같은 정책을 쓰면
     * 실패 = "이미지가 한 장도 없는 200 응답"이 되어, 게이트가 줄이려던 브라운아웃이 오히려
     * 최악의 형태로 나타난다. 그래서 응답은 "이미지 전부" 아니면 "명확한 503" 둘 중 하나가
     * 되도록 바꿨다 — 이러면 k6의 http_req_failed가 그대로 완전성 지표가 된다.
     */
    public CourseSignature signCourseScope(Long courseId) {
        ...
        try {
            return submitAndJoin(courseId);
        } finally {
            admission.release();
        }
    }

    private CourseSignature submitAndJoin(Long courseId) {
        CompletableFuture<CourseSignature> future;
        try {
            future = CompletableFuture.supplyAsync(
                () -> cloudFrontService.signCourseScope(courseId), executor);
        ...
        // 대기자만 쳐내고 작업은 남겨 오히려 동시 부하를 늘린다. 대신 join에 데드라인을 걸어
        ...
        try {
            ...
            log.warn("CloudFront 코스 스코프 서명 실패 (courseId={})", courseId, cause);
            throw new BusinessException(CloudFrontErrorCode.FAIL_GENERATE_SIGNED_URL, cause);
        }
    }
}
