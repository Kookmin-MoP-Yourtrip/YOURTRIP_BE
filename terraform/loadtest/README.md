...
**콘솔이나 AWS CLI로 리소스의 형상을 직접 바꾸지 않는다.** state가 S3 원격 backend에 있든(#157) 로컬에 있든, terraform을 우회한 변경은 state에 기록되지 않고 그대로 drift(불일치)가 된다.
...
- **`rds_password`, `jwt_secret` 등 민감값은 `terraform.tfstate`에 평문으로 저장된다.** state는 S3 원격 backend에 있고 SSE-S3로 암호화되지만(#157), 그것은 저장 시 암호화일 뿐 값 자체는 state 안에 평문으로 들어 있다. 테스트 종료 후 `terraform destroy`로 리소스 자체를 없애는 것이 여전히 최선의 완화책이다.
