...
- ~~**CloudWatch Agent 130MB를 측정 목적의 상수로 취급했다.** 운영에서 이 에이전트를 켤지 말지는 이 문서의 범위 밖이고, 끄면 예산이 그만큼 늘어난다.~~
  → **[#121](https://github.com/Kookmin-MoP-Yourtrip/YOURTRIP_BE/issues/121)에서 해소됐다. 다만 결론은 이 문장의 기대와 반대다.**
  - 운영 박스에서 CloudWatch Agent를 Grafana Alloy로 교체하고 **같은 인스턴스에서 나란히 실측**했다([monitoring-config/verification.md](../monitoring-config/verification.md)).
  - **"끄면 예산이 130MB만큼 늘어난다"가 성립하지 않는다.** CloudWatch Agent를 껐을 때 `MemAvailable`이 돌아온 양은 **45.7MB**였다. RSS 130MB 중 65%가 매핑된 바이너리(파일 백업 페이지)라 애초에 `MemAvailable`에 이미 포함돼 있었기 때문이다. **RSS를 예산의 `OS` 항에 그대로 더하거나 빼면 안 된다.**
  - §2의 표(프로세스별 RSS)를 그런 용도로 읽지 않도록 주의한다. 그 표는 **어느 프로세스가 큰지**를 보여줄 뿐이고, **예산에 들어가는 값은 언제나 `MemTotal − MemAvailable` 하나**다.
  - 그리고 **운영 박스는 에이전트를 하나도 켜지 않아도 `OS`가 300.2MB였다.** 이 문서의 321MB(CWA 포함)가 그 박스에서 재현되지 않는다는 뜻이라, `-Xmx1024m`은 어떤 에이전트 구성으로도 예산식을 만족하지 못한다(`OS ≤ 220MB`가 필요하다).

- **§2의 321MB는 이 부하테스트 박스의 값이고, 다른 박스에 그대로 옮겨 쓸 수 없다.** 위 실측에서 같은 스펙(t3.small·AL2023·`MemTotal` 1,959,220kB)의 운영 박스가 CloudWatch Agent를 켠 채로 **345.9MB**를 냈다. 25MB 차이의 원인은 규명하지 않았다 — `systemd-journald`가 여기서는 68.7MB인데 운영 박스에서는 12.8MB인 것처럼 상주 프로세스 구성이 다르다. **`OS` 항은 박스마다 다시 재야 하는 값이다.**
