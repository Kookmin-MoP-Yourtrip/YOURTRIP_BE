...
- **비교 대상이 CloudWatch Agent 하나뿐이었고, 더 가벼운 대안을 아예 후보에 올리지 않았다.** [README.md](README.md)의 후보 비교는 Alloy를 **"Prometheus agent + node_exporter + Promtail" 3-프로세스 구성**과 견주고 **프로세스 개수**를 근거로 삼았다. 그런데 **Fluent Bit은 `node_exporter_metrics`·`prometheus_scrape` 입력과 `prometheus_remote_write`·`loki` 출력으로 같은 일을 한 프로세스에서 한다.** C로 작성돼 GC 힙이 없으므로, 이번에 실제 비용으로 확인된 익명 페이지(Alloy 56.3MB)가 훨씬 작을 가능성이 높다.
  ...
  - **사후에 검토해 기각했다 (→ [README.md](README.md)).** 호스트 지표 커버리지는 우려보다 넓었다 — 지원 컬렉터 20종에 `meminfo`·`cpu`·`vmstat`·`stat`·`systemd`가 모두 있고 빠지는 것은 PSI뿐이다. 깨지는 쪽은 **앱 지표**였다. `prometheus_scrape`가 Spring Boot의 `/actuator/prometheus`를 읽다 **SIGSEGV로 죽고**(HikariCP 라벨과 과학적 표기가 원인으로 지목됨, [fluent-bit#5894](https://github.com/fluent/fluent-bit/issues/5894)), **히스토그램 버킷이 손상돼 `histogram_quantile()`이 틀린 값을 낸다**([fluent-bit#8919](https://github.com/fluent/fluent-bit/issues/8919)). 둘 다 미해결로 닫힌 신고이지 확인된 결함은 아니지만, **판정선까지 111.7MB가 남은 상황에서 메모리를 더 아끼자고 살 위험이 아니다.**
