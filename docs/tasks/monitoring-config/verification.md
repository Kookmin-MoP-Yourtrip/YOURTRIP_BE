...
**(나) RSS는 "회수 불가량"이 아니다** — 아래 표가 그 증거다.

> ⚠️ **여기서 한 번 잘못 짚었다가 데이터로 교정했다.** 처음에는 "RSS 전부를 Alloy 몫으로 본 것이 틀렸다"고 적었는데, **그 정정 자체가 틀렸다.** `smaps_rollup`을 보면 `Pss` 245,677 kB / `Rss` 248,380 kB로 **98.9%가 Alloy 단독 소유**이고, 다른 프로세스와 나눠 쓰는 것은 libc 등 `Shared_Clean` 3,760 kB뿐이다. CloudWatch Agent도 99.8%가 단독이다.
>
> **즉 240MB는 명백히 Alloy의 것이다.** 귀속은 애초에 문제가 아니었다. 문제는 **그 값을 `-Xmx` 예산의 `OS` 항에 그대로 더한 것**이다 — 예산이 묻는 것은 "누구 것인가"가 아니라 "OOM 없이 힙에 얼마를 줄 수 있는가"이고, 그 답에는 커널이 회수하는 페이지가 들어가지 않는다.
...
- **비교 대상이 CloudWatch Agent 하나뿐이었고, 더 가벼운 대안을 아예 후보에 올리지 않았다.** [README.md](README.md)의 후보 비교는 Alloy를 **"Prometheus agent + node_exporter + Promtail" 3-프로세스 구성**과 견주고 **프로세스 개수**를 근거로 삼았다. 그런데 **Fluent Bit은 `node_exporter_metrics`·`prometheus_scrape` 입력과 `prometheus_remote_write`·`loki` 출력으로 같은 일을 한 프로세스에서 한다.** C로 작성돼 GC 힙이 없으므로, 이번에 실제 비용으로 확인된 익명 페이지(Alloy 56.3MB)가 훨씬 작을 가능성이 높다.
  - **개수를 근거로 삼은 것이 잘못이었다.** 프로세스가 둘이면 런타임도 둘이므로 "1개가 항상 가볍다"는 자동으로 참이 아니고, 반대로 **모노리스 하나가 경량 프로세스 여럿보다 무거울 수 있다.** 실제로 Alloy의 `Private_Clean` 182MB는 이 박스에서 쓰지도 않는 컴포넌트를 전부 번들한 대가다.
  - **사후에 검토해 기각했다 (→ [README.md](README.md)).** 호스트 지표 커버리지는 우려보다 넓었다 — 지원 컬렉터 20종에 `meminfo`·`cpu`·`vmstat`·`stat`·`systemd`가 모두 있고 빠지는 것은 PSI뿐이다. 깨지는 쪽은 **앱 지표**였다. `prometheus_scrape`가 Spring Boot의 `/actuator/prometheus`를 읽다 **SIGSEGV로 죽고**(HikariCP 라벨과 과학적 표기가 원인으로 지목됨, [fluent-bit#5894](https://github.com/fluent/fluent-bit/issues/5894)), **히스토그램 버킷이 손상돼 `histogram_quantile()`이 틀린 값을 낸다**([fluent-bit#8919](https://github.com/fluent/fluent-bit/issues/8919)). 둘 다 미해결로 닫힌 신고이지 확인된 결함은 아니지만, **판정선까지 111.7MB가 남은 상황에서 메모리를 더 아끼자고 살 위험이 아니다.** **기각 범위는 통합 에이전트로서다** — 두 결함이 모두 지표 경로의 것이라, 로그 수집기로만 쓰는 구성에는 걸리지 않는다.
  - **그래도 이 한계는 남는다 — 기각 근거를 채택 시점이 아니라 사후에 만들었다.** 당시 비교표는 Fluent Bit을 **알고 기각한 것이 아니라 몰라서 뺀 것**이다. 결과가 같다고 과정이 같아지지는 않는다.
  - 원래 서술의 "3-프로세스"도 부정확했다 — `node_exporter`는 지표를 **노출만 하고 긁지도 보내지도 않으므로** 스크레이퍼가 반드시 별도로 필요하다는 사실이 비교표에 드러나 있지 않다.
  - **이 항목만 성격이 다르다.** 아래 나머지는 "측정을 덜 했다"이고 이것은 **"고려를 덜 했다"**이다. 이 작업의 교훈("틀린 지표로 재고 있었다")을 끝까지 적용하면 **비교 대상 선정도 틀렸을 수 있다**까지 가야 한다. 판정 자체는 옳게 통과했지만 선택지 목록이 좁았다는 것은 별개 문제다.
