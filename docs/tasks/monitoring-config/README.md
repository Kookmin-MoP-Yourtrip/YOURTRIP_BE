...
| **(나) `ps` RSS = 그 프로세스가 시스템에서 가져가는 양** | 아니다. 운영 박스의 Alloy는 RSS 246.4MB 중 **`RssFile` 188.1MB(76%)**, `Private_Clean` 184.9MB였다. 매핑된 바이너리라 **스왑이 없어도 압력이 오면 커널이 회수한다.** 그 몫까지 "에이전트가 잡아먹은 양"으로 셌다 |
