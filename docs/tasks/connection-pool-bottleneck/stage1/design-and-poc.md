...
> 이 절의 결과는 PoC 실행 후 커밋(P2)에서 채운다.
...
| # | 요청 | 기대 | 실측 | 의미 |
|---|---|---|---|---|
| 1 | `poc-a.jpg?{QS}` | 200 | (미실행) | 와일드카드 정책 자체가 유효 |
| 2 | `poc-b.jpg?{QS}` | **200** | (미실행) | ★ 핵심 — 서명 때 언급 안 한 경로에 재사용 |
| 3 | `poc-c.jpg?{QS}` | **403** | (미실행) | 코스 경계가 실제로 잘린다 |
| 4 | `poc-b.jpg` (서명 없음) | 403 | (미실행) | `private/*` behavior에 여전히 걸린다 |
| 5 | 만료시각을 과거로 두고 1 재실행 | 403 | (미실행) | 만료 조건이 실제로 평가된다 |
| 6 | 2를 2회 호출, `x-cache` 확인 | 2회차 `Hit` | (미실행) | 쿼리스트링이 캐시 키에서 빠진다 |

**3번이 특히 중요하다** — 2번만 확인하면 "와일드카드가 통한다"만 알고 "코스 경계가 진짜 경계인가"를 모른 채 진행하게 된다.

**4번이 403이 아니라 200이면 심각한 신호다** — 경로 세그먼트가 깊어지면서 `private/*` behavior에 안 걸리고 default behavior(무서명 공개)로 새고 있다는 뜻이다.
