# CD 파이프라인 검증 — 서버 없이 확인되는 4개는 통과했다

> **10개 중 4개(C1·C2·C3·C9)를 실측해 전부 통과했다.** 나머지 6개(C4~C8, C10)는 운영 인프라가 떠 있어야 확인할 수 있어 아직 측정하지 않았다 — 결과 칸이 *(미측정)* 인 항목이 그것이다.
>
> 첫 4개를 **비용 없이** 잴 수 있었던 것은 운영이 온디맨드 모델이라 평시에 ASG가 없기 때문이다. 그 상태에서 `dev`에 머지하니 워크플로가 업로드까지만 하고 초록불로 끝났고, 그 한 번의 실행이 인증(C1)·시크릿 부재(C2)·아티팩트 규약(C3)·서버 부재 시 스킵(C9)을 동시에 증명했다. **검증하려던 상태가 마침 기본 상태였다.**
>
> 남은 6개는 `terraform/prod` apply가 필요하고 올려 두는 동안 비용이 발생하므로, 아래 실행 순서를 지켜 **서버를 한 번만 띄우고 끝낸다.**
