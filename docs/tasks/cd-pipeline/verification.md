...
>
> **원인**: 폴링이 25분 타임아웃에 걸리자 워크플로가 `cancel-instance-refresh`를 호출했다. 취소되는 순간 교체를 위해 2로 늘어나 있던 용량이 desired 1로 줄고, ASG가 종료 대상을 고르는데 **기본 종료 정책은 건강 여부를 보지 않고 가장 오래된 인스턴스를 집는다** — 그게 하필 유일하게 살아 있던 정상 인스턴스였다. 자세한 추이는 아래 "마주친 문제" 2번에 있다.
>
...
**실제로는 9단계를 돌리지 않고 파라미터만 확인해 대신했다.** `terraform destroy`로 41개 리소스를 모두 내린 뒤에도 `/yourtrip/prod/artifact_key`가 `app/5a5d515.jar`(Version 4)로 남아 있었다 — 인스턴스가 그 값을 읽는다는 것은 C6에서 이미 확인했으므로, **파라미터가 살아남는다는 사실만 보이면 전제가 성립한다.** 서버를 한 번 더 띄우지 않은 이유다. 고아 리소스(ALB·ASG·RDS·ElastiCache·EC2)는 전부 0건이었다.
...
| C7 | 실패 시 자동 중단 | **기각** | ①refresh 미성공 ②기존 인스턴스 생존 ④워크플로 빨간불은 충족. **③외부 요청 실패 0건이 깨졌다** — refresh 취소 시점에 502가 121건(3분 53초). 원인·수정은 "마주친 문제" 2번 |
...
25분이 지나 워크플로가 폴링을 포기하고 refresh를 **취소하자** 서비스가 끊겼다. 25분 내내 멀쩡하던 것이 정리 동작 한 번에 무너진 것이다. 아래 "마주친 문제" 2번이 그 전말이다.
