...
- **[#113](https://github.com/Kookmin-MoP-Yourtrip/YOURTRIP_BE/issues/113) `location`까지 내려간 슬롯은 정렬이 뒤집힌다** — 시드 후보는 `seedRank`로만 줄을 서므로 도시 전역에서 온 리뷰 1위가 권역 안 후보보다 앞에 온다. 폴백 유래 후보를 별도 그룹으로 내리는 것이 후보 수정안이고, 아직 적용하지 않았다
