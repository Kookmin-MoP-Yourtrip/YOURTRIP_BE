...
> 8-6 착수 전에 [#147](https://github.com/Kookmin-MoP-Yourtrip/YOURTRIP_BE/issues/147)의 엔티티
> 디코딩을 먼저 넣는 편이 낫다 — 네이버 `&amp;`가 후보 목록에 그대로 실려 실존 장소가 탈락하고,
> 그만큼 **장소 미확보율이 부풀려진다**(1차 지표인 지어냄률은 수동 검증이라 영향이 작다).
...
- [ ] 8-6. **파이프라인 환각률 측정** (3점 비교의 마지막 점). **선행 조건**: #147의 엔티티 디코딩 반영(측정 오염 방지), 지연 재측정과 `ai.course.budget-ms` 재검토(병합 후 최대 28.0초 / 예산 30초)
