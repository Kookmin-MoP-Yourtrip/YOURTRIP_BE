...
- **[#113](https://github.com/Kookmin-MoP-Yourtrip/YOURTRIP_BE/issues/113) `location`까지 내려간 후보가 잘못된 인기 표식을 달고 앞에 선다** — 도시 전역 질의의 1위가 `[seed 1위]`로 찍혀 나가 프롬프트의 선별 기준 2를 잘못 발동시킨다. 원인이 순서인지 표식인지는 셔플 실험으로 가른 뒤 정한다 — **위치 편향("목록의 순서 자체가 신호")은 이 저장소에서 확인된 적 없는 전제**이고, 실제 선택 228건도 정렬 품질과 교락돼 있어 지금 데이터로는 가릴 수 없다
