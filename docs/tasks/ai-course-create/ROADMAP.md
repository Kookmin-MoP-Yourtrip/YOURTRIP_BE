...
>
> 진행 상황: **1단계 진행 중.** 0단계는 OpenAI 크레딧 충전을 기다리는 0-3b(실 API 검증, 2단계의 블로커)만 남기고 완료됐다.
...
- [ ] 0-1. OpenAI API 키 — **발급 완료, 크레딧 충전 대기 중.** 키는 유효하나(401이 아닌 429 반환) 계정 크레딧이 0이라 0-3b를 실행하지 못했다. **2단계의 블로커** (1단계는 무관하게 진행 가능)
