...
>
> **진행 상황: 8단계 완료 — 3점 비교가 끝났다.** 2단계에서 모델 교체만으로 환각이 크게 주는 것을 확인했고(당시 복합 지표 25.6% → 7.5% — 이후 지표 체계가 재정립돼 현행 1차 지표는 **지어냄률 9.5%**, 성공 기준 참고) Curator 모델은 `gpt-5.6-luna`로 확정됐다. 6단계 실호출 프로브에서 **위조 강등 0건**을 확인했고, 같은 프로브가 드러낸 **자연어 `area`가 네이버 시더를 죽이는 결함**은 이슈 #110에서 해소했다([STEP-6](steps/STEP-6-agents.md) 판정 9). 7단계에서 여섯 조각이 하나의 요청으로 이어졌고, **설계가 지정한 ErrorCode 다섯 중 셋이 같은 설계의 degrade 정책에 막혀 발화할 수 없다는 것**을 확인해 둘만 만들었다([STEP-7](steps/STEP-7-pipeline.md) 판정 1). 8단계에서 파이프라인이 `POST /api/my-courses/ai`에 연결됐고, 그 E2E가 드러낸 결함 둘(#134 오지역 후보 · #135 오후에 끝나는 하루)을 이슈로 떼어 해소한 뒤 병합해 재검증했다([STEP-8](steps/STEP-8-switch.md) 판정 2). 그 재검증이 드러낸 **검증 쪽 결함 셋**(#147)도 손봤는데, **엔티티 디코딩과 업종 구제만 반영하고 이름 게이트 완화는 기준선 재채점 결과를 보고 되돌렸다** — 389건 중 회수가 0건이었고, 거짓 음성의 실체가 지점 수식어가 아니라 중간 삽입어였다([STEP-8](steps/STEP-8-switch.md) 판정 3). **8-6에서 3점 비교를 완성했다 — 파이프라인 지어냄률 0.0%**(before 9.5%). LLM 이 후보 목록 밖에서 이름을 낸 것이 525건 중 8건뿐이고 그 8건이 전수 실존이라, 구조가 값을 설명한다. 같은 실행이 지연 재측정도 겸했다(p95 29.4초 / 예산 30초)([STEP-8](steps/STEP-8-switch.md) 판정 4). **남은 것은 9~11단계다** — 9단계는 사실상 동결이므로 실질적으로는 10(캐싱)·11(기록).
...
2. **환각 장소가 사용자 코스에 실리는 비율을 낮춘다.** 실측 지어냄률은 **9.5%**, 장소 미확보율은 **21.1%**([AI-HALLUCINATION-GEMINI.md](hallucination/AI-HALLUCINATION-GEMINI.md)) — 코스 하나를 받으면 평균 10곳 중 1곳이 존재하지 않는 장소이고, 다섯에 하나는 검증을 못 넘겨 코스에 실을 수 없다. 후보를 3배로 늘리고 카카오 매칭에 **이름 일치를 필수 조건**으로 걸어, 검증을 통과하지 못한 장소는 파이프라인에 아예 존재하지 않게 만든다. **처방은 1-2에서 바뀌었다** — 원래 계획한 점수 하한선은 실측에서 역효과였다.

3. **동선·시간 배치를 LLM 추측에서 실좌표 계산으로 옮긴다.** 좌표를 확보한 뒤 완전탐색으로 최적 순열을 고르므로, "시간 겹침 없음"·"day당 식사 1회"·"동선 역주행 없음"이 프롬프트 규칙이 아니라 **알고리즘 불변식**이 된다. **3-7에서 실측했다 — 파이프라인 30요청(90 day)에서 완전탐색만 껐다 켜니 총 이동거리가 872.3km → 588.7km(−32.5%, 부호검정 p < 1e-6), 자기교차가 93회 → 14회다.** 두 팔이 같은 장소를 쓰므로 이 값이 완전탐색 단독 몫이다. 다만 **"역주행 없음"은 조건부로만 성립한다** — 남은 14건 중 13건이 식사 슬롯 2개짜리 day 이고, **식사가 1개 이하인 20 day 에서는 20회 → 1회**다. 이건 결함이 아니라 비용 함수가 선언한 우선순위 그대로다([STEP-3](steps/STEP-3-route-optimizer.md) 판정 8)
...
| before/after 비교 | 환각률 25.6%(Gemini 단일 호출, 당시 복합 지표 — 이후 지어냄률 9.5%로 재정의) | **OpenAI 단일 호출 baseline을 2단계에서 재측정**해 3점 비교 |
...
> 상세 실행 기록은 [STEP-3-route-optimizer.md](steps/STEP-3-route-optimizer.md) 참고 — 설계가 비워둔 계수 셋을 정하는 과정과, 식사 윈도우 배정의 첫 구현이 벌점의 목적과 정반대였던 사건. **3-7·3-8은 8단계 이후에 소급해 더한 항목이다**(3-6까지가 답한 것은 속도뿐이었고, 계수의 적정성은 3-8 전까지 미판정이었다).
...
- [x] 3-7. **동선 개선 효과 실측** — 파이프라인 30요청을 실제로 태우고 **최적화 직전 입력을 캡처해**, 같은 장소 집합에서 `optimize(request, 1)`(탐색 끔) 대 `optimize(request)`를 페어 비교했다. 프로덕션 코드는 한 줄도 바꾸지 않는다 — 그 이음매가 3-6 벤치마크용으로 이미 있었다. **총 이동거리 −32.5%**(90 day / 30요청 전건 성공, 중앙값 33.1%, 개선 88 · 동일 0 · 악화 2, p < 1e-6) · **자기교차 93 → 14** · 역행 꼭짓점 239 → 142 · 구간당 평균 2.01 → 1.36km. **거리만 최소화하면 −41.7%까지 가므로 그 차이 9.2%p(79.8km)가 식사 시간창에 지불한 값이다.** 두 가지가 드러났다: ① **남은 교차 14건 중 13건이 식사 2개짜리 day** 이고 양보율도 그 칸에서만 15.7%로 치솟는다(1개 3.1% / 0개 0.0%) ② **식사가 없는 day 에서는 양보율이 정확히 0.0%** 로, 완전탐색이 거리 최소해를 실제 코스에서 찾아낸다는 독립 검증이 됐다([STEP-3](steps/STEP-3-route-optimizer.md) 판정 8)
- [x] 3-8. **식사 벌점 계수 스윕** — 3-7이 남긴 "`MEAL_PENALTY_PER_MIN = 2.0`이 적정한가"에 답한다. 같은 CSV 를 계수만 12가지로 바꿔 돌렸고 **LLM 비용은 0**이다(3-7이 채집과 계산을 갈라 둔 덕). 거리와 식사 시각은 맞바꾸는 관계라 "최적 λ"는 측정이 아니라 선호가 정하므로, **바꿀 근거를 (a) 지배 (b) 포화 둘로 미리 한정하고 둘 다 아니면 기각**하기로 못 박았다. **판정은 기각 — 현행 2.0을 유지한다.** 지배하는 λ 가 전량·FAMOUS·MINOR 어디에도 없고, 포화가 시작되는 지점이 전량 2.00 · MINOR 1.75 라 현행이 낭비인 경우도 없다. 얻은 것 셋: ① **2.0은 90 day 전부에서 식사 위반을 정확히 0으로 만드는 가장 작은 격자점**이고 위로 16.0까지 산출이 한 바이트도 안 바뀐다(튜닝에 민감한 값이 아니다) ② **교환비가 단조 증가해 마지막 2분의 한계 가격이 0.456 km/분**인데 λ=2.0의 지불 의사 한도가 0.500이라, 판정 1이 손으로 정한 "1km = 2분"이 왜 맞았는지가 숫자로 드러났다 ③ **λ=0에서 자기교차가 90 day 전부 0**이라 판정 8의 상관이 인과로 확인됐다([STEP-3](steps/STEP-3-route-optimizer.md) 판정 9)
...
> 상세 실행 기록은 [STEP-6-agents.md](steps/STEP-6-agents.md) 참고 — 실호출 프로브가 후보 공급의 절반이 죽어 있다는 것을 드러냈다(판정 9).

- [x] 6-5. **`duration` 키워드 처리 방침 결정** — **프롬프트에서 제외하고 모순만 로그로 남긴다.** 여행 일수의 정본은 요청의 `days`이고, 같은 사실을 두 경로로 보내면 어긋났을 때 모델이 어느 쪽을 따를지 통제할 수 없다. 보정하지 않는 이유는 **고칠 대상이 코스가 아니라 프론트의 입력 UX일 수 있어서**다. 일수가 모호한 `주말`·`장기`는 판정하지 않는다. `buildKeywordsJson`은 건드리지 않는다(하네스가 글자 단위로 고정해야 한다)
- [x] 6-1. `PromptLoader` — 프롬프트를 `resources/prompts/*.md`로 분리하고 **생성자에서** eager 로드(`@PostConstruct`가 아니다 — `AiCourseMetrics`의 0 등록과 같은 이유로, 스프링 없이 조립하는 테스트에서 조용히 빠지면 안 된다). 파일이 없으면 **애플리케이션 기동이 실패**한다(실측 확인). 플레이스홀더는 **명명 기반 `{{location}}`**이고, **표기가 틀린 자리도 로드 시점에 잡는다**. 응답 JSON 스키마도 같은 방식으로 리소스에 뒀다
- [x] 6-2. `PlannerAgent` — 컨셉·제목·day별 권역(`area`)·**권역 안 랜드마크(`anchor`, 필수 문자열 1개)**·슬롯 구성. **장소명은 한 개도 생성하지 않는다**(`anchor`는 검색 기준점일 뿐 코스에 들어가는 장소가 아니다). `area`는 자연어 그대로 둔다 — 시군구로 바꾸면 day 단위 locality를 잃는다(설계 문서의 후보 공급 "area → 좌표"). 결정론적 기본 플랜은 `anchor = location`. Planner를 별도 단계로 두는 이유는 "단계 분할"이 아니라 **Curator를 day별 병렬 실행하려면 day별 권역이 먼저 확정돼야 하기 때문**이다. **`theme`·`dayStartTime`을 계약에 더했다** — `dayStartTime`은 표시용이 아니라 최적화 입력이다(시작이 밀리면 식사 시간창 벌점이 달라져 최적 순열이 바뀐다). **`dayEndTime`은 받지 않는다**(3-5의 축소→드롭이 살아나 장소가 조용히 사라진다). **실패는 그대로 올린다** — 기본 플랜으로 degrade할지는 7-3의 판단이다
- [x] 6-3. Planner 출력 구조 검증 — day 수 불일치·MEAL 누락·슬롯 개수 초과를 **코드로 보정**한다(LLM 재호출 없음). `anchor`가 비면 `area` 텍스트로 대체(지오코딩 캐스케이드 4-8이 이어받는다). **day 번호는 응답값이 아니라 위치로 다시 매겨** 중복·0-based·비연속을 한 규칙으로 흡수한다. 슬롯 상한 6은 취향이 아니라 **3-6 벤치마크가 정한 값**이다(`n=8`이면 완전탐색이 지연 예산을 넘는다). **"day당 식사 1회"가 프롬프트 지시문에서 코드 불변식으로 옮겨온 지점이다**
- [x] 6-4. `CuratorAgent` — day별 병렬, 슬롯당 후보 3개를 **선호 순서로**. 다른 day는 모른다. **역할은 "회상"이 아니라 "선별"이다** — 입력에 5-8의 후보 목록(`[seed n위] 이름 · styleTags · distanceKm`)이 들어가고 출력은 `source` + `listIndex` + `placeName`. 선별 규칙은 설계 문서의 우선순위대로 프롬프트에. **응답 스키마의 루트는 반드시 객체여야 한다**(0-3에서 최상위 배열이 400으로 거부되는 것을 확인했다). **nullable `listIndex`가 strict 모드를 통과하는 것은 실호출로 확인했다.** 목록 번호는 **0-based**(`CandidateSlot.at`이 리스트 인덱스를 그대로 쓴다). 같은 타입이 여러 자리면 목록을 한 번만 싣는다. **스타일 태그는 enum 순서로 정렬해야 한다** — `Set.copyOf`의 반복 순서가 실행마다 달라져 정렬하지 않으면 같은 입력에 프롬프트가 매번 바뀐다. **개별 day 실패는 그 day만 비우고 예외를 올리지 않는다**
- [x] 6-7. **`SEEDED`/`LISTED` 위조 강등 검증 (코드)** — `listIndex` 범위, 목록 항목 상호명과 `placeName` 일치, 슬롯 타입 일치를 검증하고 하나라도 어긋나면 `SUGGESTED`로 **강등**(버리지 않는다 — 실존할 수 있다). "재검증 생략"의 전제는 좌표·`kakaoId`를 LLM이 옮겨 적는 게 아니라 코드가 목록에서 승계하는 것이므로, 응답 스키마에 좌표·id 필드를 두지 않는다. 메트릭 `ai.candidate.demoted{reason}` — **강등만 센다**(자리 번호가 범위 밖이거나 상호명이 빈 **폐기**는 다른 사건이라 섞지 않는다). **`GroundingStage`가 아니라 Curator 출력 직후의 순수 검증기에 뒀다** — 후보 풀과 LLM 응답을 함께 보는 유일한 지점이고, 그라운딩의 책임이 "실존 확인 + 위조 검증"으로 섞이지 않는다. **출처는 모델이 아니라 목록이 정한다**(이 값이 5-6 메트릭의 `source` 태그가 된다). 슬롯 타입 일치는 별도 규칙이 아니라 조회 방식이 보장한다
- [x] 6-6. 프롬프트에서 사라진 규칙 확인 — 시간 배치·동선·중복·스키마 강제는 이제 코드가 보장하므로 프롬프트에 남기지 않는다. 금칙어 테스트로 고정했다. **다만 "약 45줄이 사라진다"는 합계로는 성립하지 않는다** — 사라진 45줄은 실제로 한 줄도 남지 않았지만, 권역 설계와 후보 선별이라는 **기존에 없던 책임**이 더해져 두 프롬프트 합계는 95줄 → **119줄**이다. 실제로 줄어든 것은 **요청마다 새로 조립되는 부분**(95줄 → 16줄)이고 나머지는 system으로 갈려 컨텍스트 캐싱의 대상이 된다
...
> 상세 실행 계획·판정은 [STEP-8-switch.md](steps/STEP-8-switch.md) 참고 — **8-4는 E2E(8-5) 뒤로 재배열했다**(revert 안전성).
>
> **진행 상황: 8단계 완료.** 8-1~8-5 완료 후 E2E가 드러낸 결함 둘을 이슈로 떼어 해소했고
> (#134 오지역 후보 · #135 저녁까지 채우기), `dev-ai-course`를 병합해 같은 조건으로 재검증했다
> (장소 83 → 104개, 45km 초과 4 → 0건 / [STEP-8](steps/STEP-8-switch.md) 판정 2). **다만 그
> 병합으로 "이 커밋만 revert하면 된다"는 위 전제는 끝났다** — 스위치 커밋 위에 99커밋이 쌓였다.
> [#147](https://github.com/Kookmin-MoP-Yourtrip/YOURTRIP_BE/issues/147)은 8-6 착수 전에 해소했다 —
> 네이버 `&amp;`가 후보 목록에 그대로 실려 실존 장소가 탈락하면 그만큼 **장소 미확보율이
> 부풀려지기** 때문이다(1차 지표인 지어냄률은 수동 검증이라 영향이 작다). 함께 시도했던 이름 게이트
...
- [x] 8-3. **삽입 순서 = 표시 순서 보장** — `DaySchedule.places`에 `@OrderBy("id ASC")`가 걸려 있고 별도 `sequence` 컬럼이 없다. 최적화된 순서 그대로 `save()`해야 동선 순서가 재현된다
- [x] 8-4. `global/gemini` 패키지 삭제 + `GEMINI_API_KEY` 제거 (`.env.example`, `application.yml`)
- [ ] ~~8-7. **사용자 삭제 로그 기록을 스위치와 함께 켠다**~~ → **제외.** 포트폴리오 저장소라 실제 서비스 운영·사용자 피드백 수집 계획이 없다 — 삭제 이벤트가 쌓일 사용자가 없으므로 "소급 불가"가 성립하지 않는다. 파급: `Place`에 `source`/`modifier` 영속화가 불필요해져 이 단계의 스키마 변경이 0건이 되고, **9단계 착수 조건 중 삭제율 비교 둘의 데이터 소스가 사라진다**(9단계 헤더 참고). 근거는 [STEP-8](steps/STEP-8-switch.md) "착수 시점에 확정한 것"
- [x] 8-5. E2E 검증 — 실제 요청으로 코스 생성, 좌표·시간·순서 확인
- [x] 8-6. **파이프라인 환각률 측정** — **지어냄률 0.0%**(3점 비교 완성). 30요청 전건 성공·장소 525개. 채점부를 `HallucinationScoring`·`HallucinationArtifacts`·`HallucinationReport`로 떼어 두 하네스가 **같은 채점기를 쓰게** 했고(값 불변은 재채점 389행 대조로 확인), 신설 `AiPipelineHallucinationBenchmarkTest`가 파이프라인 출력에 baseline과 동일한 판정을 건다. **예산은 운영값 30초가 아니라 180초로 뒀다** — 최대 지연이 예산에 붙어 있어 운영값으로 재면 잘린 요청의 장소가 분모에서 통째로 빠진다. 대신 `elapsedMs`로 역산해 **지연 재측정도 겸했다(p50 22.3초 · p95 29.4초 · 30초 초과 1/30)**. 실행기를 `AsyncConfig` 운영 풀로 맞추지 않으면 병렬 지점이 순차로 퇴화해 지연이 41.9초까지 부풀었다. **네이버 rate limit 12회는 fail-open이 흡수했고 재측정하지 않았다**(선택적 재측정 자체가 측정을 오염시킨다). 근거는 [STEP-8](steps/STEP-8-switch.md) 판정 4

### 9. `PlaceSignalStage` (3층 인기도 + 4층 속성 추출) — **조건부**

동작 변화 있음(품질). 8단계까지로 이미 확실히 낫기 때문에(환각 차단 + 실좌표 동선 + 인기 시드) **순수 부가가치이고, 그 부가가치가 실재하는지는 측정된 바 없다.** 그래서 "일단 만들어 플래그로 붙이는" 게 아니라 **아래 조건 중 하나가 실측되면 그때 만든다.** 조건은 전부 8-7의 삭제 로그에서 나온다. **8-7이 제외되면서 삭제율 비교 조건 둘의 데이터 소스가 사라졌고, 남은 둘도 충족 전망이 없어(서술어 매칭은 4-2에서 통과, 골든 데이터셋 인프라는 범위 제외) 이 단계는 사실상 동결이다.**
...
|---|---|---|---|
| Gemini 단일 호출 | 완료 | — | **9.5%** (상한 17.9% · 장소 미확보율 21.1%) |
| OpenAI 단일 호출 | 2단계 직후 | 모델 교체 효과 | **(참고) 1.08%** — 산출물 미보존, 옛 기준 측정 |
| **OpenAI 파이프라인** | **완료(8-6)** | 파이프라인 구조 효과 | **0.0%** (상한 1.3% · 장소 미확보율 19.8%) |

**파이프라인 값이 0.0%인 것은 구조가 설명한다** — 525건 중 **LLM이 후보 목록 밖에서 이름을 낸 것이 8건(1.5%)뿐**이고, 그 8건을 전수 판정한 결과 모두 실존했다. 나머지 517건은 네이버·TourAPI가 준 상호를 코드가 승계한 것이라 원리적으로 지어낼 여지가 없다. Curator가 `SUGGESTED`로 시도한 57건 중 15건은 그라운딩에서 탈락했다 — **지어낸 이름이 코스에 도달하기 전에 걸러지는 경로가 실측으로 확인됐다.**

> **읽을 때 주의 둘.** ① **자동 프록시는 오히려 올랐다**(11.6% → 13.0%). `SEEDED`·`LISTED`는 카카오를 부르지 않고 승계하므로 그쪽의 `NO_RESULT`는 환각이 아니라 카카오 커버리지 구멍이고, 좌표는 있고 `placeUrl`만 빈다. 파이프라인에서 "장소 미확보율"이라는 이름은 사실과 어긋난다 ② **표본 50건에서 관측되지 않았다는 뜻이지 0이 증명된 것은 아니다** — 층별 0/10이라 통계적 상한이 넓다. 더 강한 근거는 `SUGGESTED` 전수 8/8이다. 상세는 [STEP-8](steps/STEP-8-switch.md) 판정 4
...
- **지표의 산출 정의를 고정한다** — 아래 절차를 그대로 따라야 측정점이 같은 것을 재게 된다. 근거·한계·재현 조건은 [AI-HALLUCINATION-GEMINI.md](hallucination/AI-HALLUCINATION-GEMINI.md)의 "지표의 정의"와 "재측정 재현 조건"에 있다. **8-6 착수 전에 그 두 절을 먼저 읽는다**
  ...
  >
  > **[정정] before 지어냄률 10.1% → 9.5%** (2026-08-27). 8-6 착수와 함께 미기입으로 남아 있던 `manual-verification-rescore-*.csv`(새 층 기준 정식 층화 50건)를 판정해 재산출했다. **채점 결과 389행과 검증 함수는 그대로이고 표본만 바뀌었다** — 옛 값은 옛 밴드로 뽑은 104건을 새 층에 재조인한 99건에서 나와 층별 11~28건으로 불균등했다. 세탁 통과율은 0.6%로 동일하다. 상세는 [AI-HALLUCINATION-GEMINI.md](hallucination/AI-HALLUCINATION-GEMINI.md)의 `[정정]` 블록
  >
  > ~~**8-6 주의**: 현행 하네스는 단일 호출 전용이라 파이프라인 경로를 그대로 잴 수 없다~~ → **해소.** 8-6에서 채점·산출물 로직을 `HallucinationScoring`·`HallucinationArtifacts`·`HallucinationReport`로 떼고 `AiPipelineHallucinationBenchmarkTest`를 신설했다. 두 하네스가 같은 채점기를 쓰므로 절차 동일성이 구성으로 유지된다
- BASELINE 문서가 명시한 한계를 그대로 승계한다 — **LLM 응답은 비결정적이라 수 %p 차이는 반복 측정이 필요하고**, 카카오에 미등록된 실존 업소는 원리적으로 환각과 구분할 수 없다

**부가 지표** — 아직 목표만 있는 것과 실측이 끝난 것이 섞여 있다.

| 지표 | before | 목표 · 실측 |
|---|---|---|
| 코스 총 이동거리 | 872.3 km (90 day, 완전탐색 OFF) | **588.7 km — −32.5%** (3-7 실측) |
| 동선 자기교차 | 93회 | **14회** — 남은 13건이 식사 2개짜리 day 다 (3-7 실측) |
...
| 요청 지연 (파이프라인) | — | **p50 22.3초 · p95 29.4초 · max 30.1초** (8-6 실측, 30요청). 운영 예산 30초 초과 1건 — 11-2의 근거 |
| `SUGGESTED` 비중 | 단일 호출은 전량 | **8 / 525 = 1.5%** (8-6) — 후보 공급이 LLM 의 자유 생성을 덮은 비율 |
...
- ~~**`duration` 키워드 처리 방침**~~ → **6-5에서 결정.** 프롬프트에서 제외하고 모순만 로그로 남긴다
- ~~**Planner의 자연어 `area`가 네이버 시더를 죽인다**~~ → **이슈 #110에서 해소.** 권역명을 검색 가능한 지명으로 줄이고(구분자 앞부분 + 권역 접미어 제거), 모자라면 `anchor` → `location` 순으로 더 모은다. 여섯 전략을 12권역 × 3슬롯 × 3회로 비교해 골랐고, `location`만 쓰는 안은 0건이 사라지는 대신 **거리가 열 배**라 탈락했다. 근거는 [STEP-6](steps/STEP-6-agents.md) 판정 9
- ~~**[#113](https://github.com/Kookmin-MoP-Yourtrip/YOURTRIP_BE/issues/113) `location`까지 내려간 후보가 잘못된 인기 표식을 달고 앞에 선다**~~ → **표식은 해소했다.** `SeedScope`(`AREA`/`ANCHOR`/`LOCATION`)로 단계를 후보에 실어 광역 후보를 `[seed·광역]`으로 표기하고, 프롬프트의 선별 기준 2에서 뺐다. 셔플 실험(LLM 6회)은 1순위 일치 55.6%로 **사전에 못 박은 미판정 구간**에 들어 위치 편향을 확정하지 못했다 — 그 사실을 [STEP-6](steps/STEP-6-agents.md) 판정 12에 남겼다
- **광역 후보가 목록 0번을 차지한다(`CandidateOrdering` ① 그룹)** — #113을 조사하며 LLM 없이 32권역으로 재니 **광역이 섞인 슬롯 4개가 전부 0번**이었고, 셋은 같은 슬롯에 훨씬 가까운 후보를 두고 앞에 섰다(0.1km 후보 대 **14.6km 마곡사**). 원인은 시드 그룹을 `seedRank`로만 세우고 **거리를 tie-break에도 안 넣는** 것이다. 다만 **꼬리로 내리는 처방은 위험하다** — 상권 없는 권역에서는 그 후보가 유일하게 실존이 확인된 것이다. 근거는 [STEP-6](steps/STEP-6-agents.md) 판정 12
- **[#112](https://github.com/Kookmin-MoP-Yourtrip/YOURTRIP_BE/issues/112) 후보가 3건에 못 미치는 슬롯이 18% 남는다** — `anchor` 단계의 이득에 구조적 상한이 있다(관측 32건 중 20건은 `anchor`가 `area` 정규화 결과와 같아 건너뛴다). 그 20건 중 18건에 **아직 쓰지 않는 두 번째 지명**이 있다. `area`를 한 번 더 파싱할지, Planner에게 `searchTerms`를 직접 받을지가 갈림길이다
- **`AreaGeocoder`의 캐스케이드 2단계(`{location} {area}`)도 같은 결함을 갖는다** — 그 javadoc이 스스로 인정하고 있고, `anchor`가 실패했을 때만 타는 가려진 경로라 지금은 드러나지 않는다. #110에서는 **측정만 하고 손대지 않았다**(고치면 4-8의 단계별 `outcome` 태그 의미가 바뀐다)
...
- [AI-HALLUCINATION-GEMINI.md](hallucination/AI-HALLUCINATION-GEMINI.md) — 환각률 baseline 실측 (before 지어냄률 9.5% · 장소 미확보율 21.1%)
- [route/artifacts/README.md](route/artifacts/README.md) — 3-7 동선 개선 효과와 3-8 계수 스윗의 원본 산출물. 목표 3의 근거
