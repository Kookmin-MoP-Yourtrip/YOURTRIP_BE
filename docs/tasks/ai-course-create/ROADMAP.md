...
>
> **진행 상황: 8-6만 남았다.** 8-1~8-5 완료 후 E2E가 드러낸 결함 둘을 이슈로 떼어 해소했고
...
- [ ] 8-6. **파이프라인 환각률 측정** (3점 비교의 마지막 점). **선행 조건**: ~~#147의 엔티티 디코딩 반영(측정 오염 방지)~~ → 해소했고, 검증 함수(`lookupBestPlace`)는 결국 손대지 않아 측정 도구가 그대로다([STEP-8](steps/STEP-8-switch.md) 판정 3) / 지연 재측정과 `ai.course.budget-ms` 재검토(병합 후 최대 28.0초 / 예산 30초)
