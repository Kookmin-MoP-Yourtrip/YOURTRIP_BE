...
>
> **진행 상황: 6단계 완료.** 2단계에서 복합 환각률 25.6% → **7.5%**(모델 교체 효과)를 확인했고 Curator 모델은 `gpt-5.6-luna`로 확정됐다. 6단계 실호출 프로브에서 **위조 강등 0건**을 확인했고, 같은 프로브가 드러낸 **자연어 `area`가 네이버 시더를 죽이는 결함**은 이슈 #110에서 해소했다 — 권역명 정규화 + 0건 재질의로 `SUGGESTED` 비율이 **40%/50% → 0%/3.3%** 가 됐다([STEP-6](steps/STEP-6-agents.md) 판정 9). **다음은 7단계(파이프라인 오케스트레이터).**
...
- ~~**Planner의 자연어 `area`가 네이버 시더를 죽인다**~~ → **이슈 #110에서 해소.** 권역명을 검색 가능한 지명으로 줄이고(구분자 앞부분 + 권역 접미어 제거), 그래도 0건이면 도시 전체로 재질의한다. 여섯 전략을 12권역 × 3슬롯 × 3회로 비교해 골랐고, `location`만 쓰는 안은 0건이 사라지는 대신 **anchor 거리가 열 배**라 탈락했다. 근거는 [STEP-6](steps/STEP-6-agents.md) 판정 9
