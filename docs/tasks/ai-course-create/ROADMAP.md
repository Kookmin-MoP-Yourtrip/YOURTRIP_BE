...
>
> **진행 상황: 6단계 완료.** 2단계에서 복합 환각률 25.6% → **7.5%**(모델 교체 효과)를 확인했고 Curator 모델은 `gpt-5.6-luna`로 확정됐다. 6단계 실호출 프로브에서 **위조 강등 0건**을 확인했지만, 같은 프로브가 **Planner의 자연어 `area`가 네이버 시더를 죽인다는 것**을 드러냈다(MEAL·CAFE 목록이 통째로 빈다). 별도 이슈이며 **8단계 스위치 전에 해소해야 한다**([STEP-6](steps/STEP-6-agents.md) 판정 9). **다음은 7단계(파이프라인 오케스트레이터).**
...
- **Planner의 자연어 `area`가 네이버 시더를 죽인다** — 가운뎃점으로 두 지명을 붙인 권역명(`황리단길·대릉원 일대`)으로는 지역검색이 0건이라 **MEAL·CAFE·SHOPPING 목록이 통째로 빈다**(관광 슬롯은 TourAPI가 메워 살아남는다). 설계의 두 요구(`area`는 자연어 그대로 / `area`는 쿼리 접두사)가 실제로 충돌한다. 5-9가 이걸 못 본 것은 그때 프로브가 `area`에 단순 지역명을 넣었기 때문이다. **별도 이슈이며 8단계 스위치 전에 해소해야 한다** — 근거와 표기별 실측은 [STEP-6](steps/STEP-6-agents.md) 판정 9
