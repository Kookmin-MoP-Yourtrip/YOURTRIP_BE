...
>
> 진행 상황: **2단계 완료**(2-1 ~ 2-6, 테스트 103개 통과 + 앱 기동 확인 + baseline 재측정 4조합 120요청 + 수동 검증 82건). 복합 환각률 25.6% → **7.5%**(luna), Curator 모델은 `gpt-5.6-luna`로 확정됐다. 다음은 3단계(`RouteOptimizer`). 0단계 검증 항목은 전부 통과했고 OpenAI·네이버 키도 발급됐다.
>
