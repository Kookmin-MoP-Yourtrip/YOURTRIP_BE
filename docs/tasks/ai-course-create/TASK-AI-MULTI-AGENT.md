...
> **[정정] 네이버 검색 API가 NAVER API HUB(네이버 클라우드 플랫폼)로 이관되면서 종량제가 됐다.**
> 무료 쿼터 전제가 사라졌으므로 위 문장은 성립하지 않는다. "LLM 토큰을 늘리지 않는다"는 부분은
> 여전히 맞지만, **네이버 호출 자체가 과금 대상**이 됐다.
>
> 규모가 작지 않다 — 코스 1건에 네이버 호출이 약 35회이고, 일 1,000요청이면 **월 약 105만 회**다.
> 0단계에서 확정한 LLM 비용이 요청당 약 4.2원이므로, **네이버 단가가 호출당 0.12원만 넘어도
> 네이버가 LLM보다 비싸진다.** 3층이 "가장 비용 효율이 좋은 수단"인지는 이제 단가를 확인해야
> 답할 수 있는 문제이고, 동시에 §13 10단계의 캐싱(`naver:blog:{sha1}`, TTL 7일)이 "지연 감소"가
> 아니라 **비용 절감 수단으로 격상**된다.
>
> 다행히 과금 사고를 막는 장치는 설계에 이미 있다 — 네이버는 hard fail이 아니라 fail-open이므로
> (§9) 콘솔에서 이용 한도를 걸어 호출이 막혀도 코스 생성은 계속되고 인기도·속성 신호만 빠진다.
>
> 새 엔드포인트·인증 헤더·발급 절차와 단가 확인 항목은
