...
| 밴드 | 점수 | 전체 비중 | CORRECT | 불량(LAUNDERED+WRONG_MATCH) |
|---|---|---|---|---|
...
- **E2E 검증** — 로드맵 적용 원칙이 "API 동작에 영향을 주는 단계(1, 8, 9)는 실제 요청으로 검증"을 규정한다. 확인할 것: ① 매칭 성공 장소의 좌표 저장 ② 매칭 실패 장소가 `0.0/0.0`이 아니라 `null`로 저장·응답되는지 ③ `keywords` 누락 요청이 500이 아니라 400인지
- **FE 공유** — 응답의 `latitude`/`longitude`가 nullable이 됐다
