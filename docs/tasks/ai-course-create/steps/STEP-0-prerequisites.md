...
설계 문서의 도입 순서의 도입 순서는 1단계(기존 결함 수정)부터 시작한다. 하지만 로드맵을 쓰면서 코드를 확인해보니 **착수 자체가 막히는 선행 조건**이 여럿 있었다.
...
이 단계에서 가장 중요한 검증이다. 설계 문서의 LLM 포트 설계·프롬프트 전략은 **"JSON 스키마를 프롬프트 텍스트가 아니라 디코딩 레벨에서 강제한다"**는 전제 위에 서 있고, 그 위에 세 가지 결론이 얹혀 있다.

- 프롬프트에서 스키마·출력예시 약 36줄이 사라진다 (프롬프트 전략)
- 파싱 실패율이 near-zero가 된다 (LLM 포트 설계)
- "의미 재시도"를 1회로 제한할 수 있다 (LLM 포트 설계)
...
**따라서 배열을 객체로 감싸는 것은 취향이 아니라 제약이다.** 도입 순서 6단계에서 CuratorAgent 응답 스키마를 설계할 때 슬롯 배열을 루트에 두면 400을 맞는다 — 설계 문서의 파이프라인 전체 구조의 예시(`{ "day": 1, "slots": [...] }`)처럼 반드시 객체로 감싸야 한다. 이걸 6단계에서 알았다면 스키마와 파싱 코드를 되돌려야 했을 것이다.
...
PlaceProfile을 nano로 내린 선택이 틀렸는지는 설계 문서의 관측 설계에 이미 있는 `ai.profile.traits{count}` 메트릭이 알려준다 — 추출률이 낮게 나오면 그때 올린다.

### 설계 문서의 비용 분석의 전제 하나가 틀렸다

비용 분석은 "입력 토큰이 9배가 되는 것이 이 설계의 최대 비용이고, 전부 4층(PlaceProfile)에서 나온다"고 했다. **토큰 수로는 맞지만 금액으로는 아니다.**
...
출력 단가가 입력의 8배라 두 비중이 어긋난다. **금액을 지배하는 것은 PlaceProfile의 입력이 아니라 Curator의 출력이다.** 이는 비용 분석의 "조건부 확장으로 약 40% 절감" 논의 전제도 바꾼다 — ATTRACTION 슬롯을 PlaceProfile 대상에서 빼는 것으로 아낄 수 있는 금액은 생각보다 훨씬 작다.

---

## 쿼터와 과금

> **[정정] 네이버 검색 API가 NAVER API HUB(NCP)로 옮겨갔다.** 이 문서의 초안은 ~~developers.naver.com에서 발급~~한다고 적었으나, 현재 검색 API는 네이버 클라우드 플랫폼의 **NAVER API HUB**로 제공된다.
>
> **다만 요금 정책은 그대로다.** 이관 사실을 확인한 직후 "pay-as-you-go"라는 홍보 문구만 보고 ~~종량제로 바뀌어 무료 쿼터가 사라졌다~~고 한 번 더 정정했는데, **공식 요금표를 확인해보니 그게 틀렸다.** 검색 API는 여전히 무료이고 일 25,000건 한도도 그대로다. 아래 표가 확인된 값이다.

| API | 한도/과금 | 요청당 호출 | 상한 |
|---|---|---|---|
| **네이버 검색 (API HUB)** | **0~775,000건 0원 / 일 최대 25,000건** | ~35 | **약 714 요청/일** |
| 카카오 로컬 | 100,000/일 (API 종류별) | ~45 | 약 2,222 요청/일 |
| OpenAI | 계정 티어별 RPM/TPM | 5 | 미확인 — 키 발급 후 확인 |

`775,000 = 25,000 × 31`이므로 월 상한은 일일 한도를 31일로 곱한 값일 뿐이고, **실질 제약은 일 25,000건 하나**다.

### 네이버가 여전히 가장 먼저 한계에 닿는다

요청당 호출 수는 카카오(45회)가 더 많지만 한도가 4배라, **네이버가 약 714 요청/일로 먼저 막힌다.** 다만 이건 서비스가 죽는 종류의 한계가 아니다 — 네이버는 hard fail이 아니라 **fail-open**이므로(설계 부분 실패 전략), 한도를 넘겨도 코스 생성은 계속되고 인기도·속성 신호만 빠진다. 품질이 이 작업 이전 수준으로 돌아갈 뿐이다.

10단계 캐싱(`naver:blog:{sha1}`, TTL 7일)이 붙으면 상한이 올라간다. 경주·제주·부산이 요청 대부분을 차지하는 특성상 히트율이 높을 것으로 보이고, 50%만 나와도 **약 1,400 요청/일**이 된다.

**설계 문서의 비용 분석의 "3층은 비용 증가가 0이고 이 설계에서 가장 비용 효율이 좋은 품질 개선 수단"이라는 평가는 유효하다.** 무료 쿼터 안에서 동작하고 LLM 토큰을 늘리지 않는다는 근거가 그대로 성립한다.
...
**일 25,000건 한도를 초과하면 `429`가 반환된다.** 4단계 `NaverBlogClient`의 fail-open 분기는 이 코드를 기준으로 잡는다 — 429는 재시도 대상이 아니라 **그날의 쿼터 소진**이므로, 전송 계층의 지수 백오프 재시도를 태우지 말고 즉시 신호 없이 진행해야 한다(설계 부분 실패 전략의 "네이버 개별 조회 실패 → 인기도·traits 없이 카카오 점수만으로 랭킹"). 여기서 재시도를 돌리면 이미 소진된 쿼터에 지연 예산만 태우게 된다.
...
- **`NAVER_CLIENT_ID` / `NAVER_CLIENT_SECRET`** — **NCP 콘솔의 NAVER API HUB**에서 발급한다(위 "NAVER API HUB 발급 절차와 스펙" 표 참고). developers.naver.com이 아니다. **API 목록에서는 "블로그(NAVER Search Blog API)" 하나만 선택하면 된다** — 3·4층이 필요로 하는 `total`·`postdate`·`title`·`description`이 블로그 조회 한 번에 전부 들어오고, "지역(Local)"은 설계 기각한 대안에서 `display` 최대 5건이라 표본이 부족해 이미 기각한 항목이다. 4단계의 블로커다
...
- [멀티 에이전트 파이프라인 설계](../멀티-에이전트-파이프라인.md) — 설계 근거(허브). 이 문서의 판정으로 갱신된 절은 [LLM 포트 설계 벤더 중립 추상화](../design/LLM-연동.md)·[비용 분석 비용](../design/운영-관심사.md)이다
- [AI-HALLUCINATION-GEMINI.md](../hallucination/AI-HALLUCINATION-GEMINI.md) — 2단계에서 재사용할 측정 하네스
