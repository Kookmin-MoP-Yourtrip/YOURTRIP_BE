...
| API | 일일 한도 | 요청당 호출 | 일일 상한 |
|---|---|---|---|
| 네이버 검색 | 25,000 | ~35 | **약 714 요청** |
| 카카오 로컬 | 100,000 (API 종류별) | ~45 | 약 2,222 요청 |
| OpenAI | 계정 티어별 RPM/TPM | 5 | 미확인 — 키 발급 후 확인 필요 |

**네이버가 가장 먼저 한계에 닿는다.** 다만 캐시(10단계, TTL 7일)가 붙으면 경주·제주·부산에 요청이 몰리는 특성상 실질 여유는 훨씬 커진다. 네이버는 hard fail이 아니라 fail-open이므로(설계 §9), 쿼터를 넘겨도 코스 생성 자체는 계속된다.
...
- **`NAVER_CLIENT_ID` / `NAVER_CLIENT_SECRET`** — developers.naver.com에서 애플리케이션 등록 후 **"검색" API 사용 설정**까지 해야 발급된다. 4단계의 블로커다
