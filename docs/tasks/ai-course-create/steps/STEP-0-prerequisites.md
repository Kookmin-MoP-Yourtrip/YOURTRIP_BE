...
요청당 호출 수는 카카오(45회)가 더 많지만 한도가 4배라, **네이버가 약 714 요청/일로 먼저 막힌다.** 다만 이건 서비스가 죽는 종류의 한계가 아니다 — 네이버는 hard fail이 아니라 **fail-open**이므로(설계 §9), 한도를 넘겨도 코스 생성은 계속되고 인기도·속성 신호만 빠진다. 품질이 이 작업 이전 수준으로 돌아갈 뿐이다.

10단계 캐싱(`naver:blog:{sha1}`, TTL 7일)이 붙으면 상한이 올라간다. 경주·제주·부산이 요청 대부분을 차지하는 특성상 히트율이 높을 것으로 보이고, 50%만 나와도 **약 1,400 요청/일**이 된다.

**설계 문서 §11의 "3층은 비용 증가가 0이고 이 설계에서 가장 비용 효율이 좋은 품질 개선 수단"이라는 평가는 유효하다.** 무료 쿼터 안에서 동작하고 LLM 토큰을 늘리지 않는다는 근거가 그대로 성립한다.
...
- [멀티 에이전트 파이프라인 설계](../멀티-에이전트-파이프라인.md) — 설계 근거(허브). 이 문서의 판정으로 갱신된 절은 [§6 벤더 중립 추상화](../design/LLM-연동.md)·[§11 비용](../design/운영-관심사.md)이다
