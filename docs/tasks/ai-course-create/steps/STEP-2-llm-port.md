...
## 2-6 측정 결과 — OpenAI 단일 호출 (1/4 조합)

**조합**: `gpt-5.6-luna` / 프롬프트지시(구조화 출력 없음) / 추론 `low` / 의미 재시도 1회
**규모**: 30요청 전량 성공, 장소 454개 검증
**산출물**: `results/*-openai-luna-prompt-relow-20260813-141510*` (원본 응답 30건 + CSV 3종 + 수동 검증 표본 40건)
...
네 조합 중 하나(`luna` × 프롬프트지시)를 돌렸다. **각 조합마다 실행 전 승인이 필요하다**(로드맵 "적용 원칙").

| 조합 | 상태 | 무엇을 갈라주나 |
|---|---|---|
| luna × 프롬프트지시 | **완료** | 3점 비교의 중간점 (모델 교체 효과) |
| luna × json_schema | 미실행 | 구조화 출력 효과 (V2) |
| nano × 프롬프트지시 | 미실행 | Curator 모델 확정 |
| nano × json_schema | 미실행 | 위 둘의 교차 확인 |

- 남은 규모: 30요청 × 3조합 = **90요청**, 조합당 약 8~12분
- 추론 강도는 **네 칸 모두 `low`로 고정**한다. 두 모델이 공통으로 받는 유일한 값이라 선택지가 아니라 제약이다(판정 6)

```bash
BASELINE_MODEL=luna BASELINE_SCHEMA_MODE=json_schema BASELINE_REASONING_EFFORT=low \
  ./gradlew benchmarkTest --tests '*AiHallucinationBaselineTest*' --rerun
```

산출물은 `results/hallucination-baseline-openai-{model}-{mode}-re{effort}-{ts}.csv`로 축이 파일명에 박힌다(`results/`는 `.gitignore` 대상이라 이름이 유일한 라벨이다).

**측정과 별개로 남은 것**

- **수동 검증 40건** — `manual-verification-*.csv`의 `verdict` 칸을 사람이 채워야 세탁된 환각률이 나오고, 그래야 25.6%와 같은 정의의 복합 지표가 완성된다. 자동으로는 잡을 수 없는 부분이다
- [ROADMAP.md](../ROADMAP.md) 성공 기준 표의 "OpenAI 단일 호출" 행
- **Curator 모델 확정** — nano 조합을 돌려야 판단할 수 있다(0단계가 2-6에 넘긴 숙제)
- OpenAI RPM/TPM 티어 — 이번 30요청에서 **429가 0건**이었다. 요청 간 5초 지연·동시 호출 1이라 느슨한 조건이었으므로, `max-concurrent-calls: 2`의 근거로 삼기에는 아직 부족하다
