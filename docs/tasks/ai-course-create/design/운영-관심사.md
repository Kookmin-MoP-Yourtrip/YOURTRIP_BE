...
1. **traits 커버리지가 좁다.** PlaceProfile은 비용상 MEAL/CAFE 슬롯에만 적용된다(비용 분석). Critic이
   ATTRACTION/VIEWPOINT/WALK 슬롯을 채점할 때는 참고할 `traits`가 없어, **Curator와 같은
   파라메트릭 지식으로 다시 한번 추측하는 것**에 가깝다 — 원칙 2가 경고한 "지식을 추가하지 않는
   재구성"이 정확히 여기서 재발한다.
2. **판단 기준이 아직 없다.** CriticAgent 설계에 "미확정"으로 남긴 것처럼, `IssueType`을 언제 부여할지에
...
3. **딸려오는 서브시스템이 작지 않다.** Critic 하나가 아니라 CandidateRefiner(후보 교체 + 재최적화)
   ...
   품질 향상"이다.
4. **효과를 측정할 인프라가 없다.** 남는 한계에서 골든 데이터셋 평가를 범위 밖으로 미뤄뒀다. 그 인프라
...
**결정: Critic·Refiner는 V1에서 제외한다.** 설계(CriticAgent 설계, CandidateRefiner 설계)는 그대로 남기고, 아래 조건 중 하나가
충족되면 재검토한다.
- 골든 데이터셋/LLM-as-judge 평가 인프라가 생겨 "Critic이 실제로 개선하는가"를 측정할 수 있을 때
- 실제 사용자 피드백에서 컨셉 미스매치 불만이 반복될 때

이 결정 시점에 V1은 Planner·Curator·Grounding·PlaceSignal(PopularityScorer+PlaceProfileAgent)·
RouteOptimizer 다섯 단계로 확정됐다.

> **[갱신] 이후 PlaceSignal도 같은 논리로 V1에서 빠졌다.** 위 1번 근거("traits 커버리지가 좁아
> Critic이 참고할 외부 근거가 없다")는 PlaceProfile이 있다는 전제 위에 쓰였는데, 후보 공급 층 도입
> 후 PlaceProfile 자체가 "효과 미측정 + 비용 확정"(위 4번과 같은 근거)으로 제외됐다. Critic
> 재검토 조건도 그에 맞춰 읽어야 한다 — Critic을 켜려면 traits가 필요하고 traits를 켜려면 4층이
> 필요하므로, **4층 → Critic 순서**로만 재검토가 성립한다. 현재 V1은 **Planner·CandidateRetrieval·
> Curator·Grounding·RouteOptimizer(+PlaceUrlEnricher)** 이다. 근거는
> ["PlaceSignal을 V1에서 제외한 이유"](지식-신호와-후보-공급.md).
