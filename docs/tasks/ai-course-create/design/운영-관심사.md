...
> Critic·Refiner는 V1 범위 밖이라 이 표에 없다. 설계상 실패 처리(CriticAgent 설계/CandidateRefiner 설계 재검토 시 참고):
...
| | 카카오 | 네이버 블로그 (3·4층) | 네이버 지역검색 (시더, 후보 공급) |
|---|---|---|---|
...
| 입력 토큰 | ~1,400 | ~13,800 (PlaceProfile의 스니펫이 대부분 + **Curator 후보 목록 day당 +600~900**, 후보 공급) |
| 출력 토큰 | ~600 | ~1,700 |
...
("Critic·Refiner를 V1에서 제외한 이유") 위 5회 기준이 실제 비용이다.
...
> **하루 약 714 코스 생성**이 상한이고, 도입 순서 10단계 캐싱(`naver:blog:{sha1}`, TTL 7일)이 붙어
> 히트율 50%가 되면 약 1,400건까지 늘어난다. **지역검색(후보 공급 층의 시더 + 스타일 modifier) ~12~18회가
> 블로그 검색과 쿼터를 합산하는지 별도 산정하는지는 아직 확인하지 못했다** — 합산이면 요청당
> ~47~53회로 상한이 약 470~530 코스로 내려간다(남는 한계의 확인 항목). 합산으로 확인되면 스타일 modifier
> 수를 슬롯당 1개로 줄이는 것이 첫 번째 조절 손잡이다. 어느 쪽이든 지역검색은 fail-open이라 상한
> 초과 시 시드·스타일 후보만 빠진다. 상한을 넘겨도 서비스가 죽지는 않는다 — 네이버는
> hard fail이 아니라 fail-open이므로(부분 실패 전략) 인기도·속성 신호만 빠지고 코스 생성은 계속된다.
>
