...
| [지식 신호 층과 후보 공급](design/지식-신호와-후보-공급.md) | 지식 신호 층 · `CandidateRetrievalStage`(후보 공급) |
...
(문제 장소 교체 + 재최적화, 최대 1회)가 이어질 수 있지만, **V1에서는 제외**했다 — 근거는 지연 예산
"Critic·Refiner를 V1에서 제외한 이유"에, 설계 자체는 CriticAgent 설계·CandidateRefiner 설계에 그대로 남겨뒀다.
...
```
MEAL 슬롯도 동일. ATTRACTION은 카카오 검색만(시더는 MEAL/CAFE 한정, 후보 공급).
...
그리고 코드 측 위조 강등 검증(후보 공급)이 추가된다. 6단계 착수 전에 이 문서 후보 공급을 다시 읽는다.
