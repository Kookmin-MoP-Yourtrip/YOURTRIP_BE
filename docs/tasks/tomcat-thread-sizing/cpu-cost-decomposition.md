...
**(a) 직접 비용은 작고, 간접이 압도한다.** 메모리를 안 건드리는 전환(c1)은 **7.2~7.7μs**다(N≥8). 같은 전환이 워킹셋 1MB에서는 **108~123μs**가 된다 — **간접이 직접의 14~16배**다. 커널 경로가 아니라 캐시 되채우기가 전환 비용의 본체다.

> c1이 T8 arm에서 독립적으로 얻은 **9.4μs**(§6-3)와 일치한다. 실제 워크로드에서 역산한 값과 합성 벤치마크가 서로를 검증한다. N=2만 15.6μs로 높은데, 스레드가 둘뿐이라 CPU가 놀고 idle→wake 왕복 지연이 얹히기 때문이다.

**(b) S=4KB 행이 기전을 직접 보여준다.** 스레드당 워킹셋을 L1d에도 들어가는 4KB로 고정하고 N만 키우면, **간접 비용은 총 워킹셋이 공유 L2(1MB)에 닿기 전까지 정확히 0이다.**
...
**(c) 앱 실측과 맞아떨어진다.** §6-4의 tomcat 그룹 증가분 135.8μs/req를 요청당 재개 횟수(3.41회)로 나누면 **재개 1회당 약 40μs**다. 위 표에서 워킹셋 256KB(16~19μs)와 1MB(84~116μs) 사이에 들어간다. 앱의 잔여분이 독립 벤치마크의 측정 범위 안에서 설명된다.
...
- **캘리브레이션은 합성 워크로드다.** 스레드마다 독립된 배열을 훑을 뿐이라 실제 앱처럼 코드·자료구조를 공유하지 않는다. 절대값을 앱에 그대로 대입할 수 없고, §6-6(c)의 대조는 자릿수 확인이다.
