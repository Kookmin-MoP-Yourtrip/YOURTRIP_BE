...
>
> **결론만 먼저**: [README.md](README.md)에 구축 **전에** 못 박은 판정 기준 12개를 모두 통과했다. 총 **134만 건의 요청 중 5xx가 0건**이었고, 스케일아웃(P11)은 임계값을 낮추지 않은 채 7분 부하로 실제 발동시켜 그 원인이 우리가 건 정책임을 AWS 활동 기록으로 확인했다.
>
...
| P10 | ALB 502 없음 | ✅ | **총 134만 건 중 5xx 0건** — `HTTPCode_ELB_502_Count`·`HTTPCode_ELB_5XX_Count`·`HTTPCode_Target_5XX_Count` 전부 데이터포인트 없음(=0) |
| P11 | 스케일아웃 발동 | ✅ | 아래 상세 |
...
k6 VUS 100, `popular-cold.js`, HTTPS 경유. 3분(발동 실패)과 7분(발동) 두 번 측정했다.

| 지표 | 3분 부하 | 7분 부하 |
|---|---|---|
| 처리량 | 2,972 req/s | **3,542 req/s** |
| 총 요청 | 535,048 | **1,487,724** |
| 실패율 | 0.00% | **0.00%** |
| p95 | 55.25ms (avg 33.51ms) | — |
| `RequestCountPerTarget` 최대 | 분당 207,155 | — |
| 스케일아웃 | ❌ 발동 안 함 | ✅ **발동** |

7분 쪽 처리량이 더 높은 것은 **중간에 인스턴스가 2대로 늘었기 때문**이다. 부하 조건은 같다(VUS 100).

임계값 대비 배율: 분당 207,155 / 30,000 = **6.9배**.
