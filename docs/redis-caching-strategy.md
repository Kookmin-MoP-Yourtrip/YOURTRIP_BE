...
- 항목 하나를 적용하면 반드시 **사용자 확인**을 받은 뒤 다음 항목으로 넘어간다.
...
### 0. 사전 준비
- [ ] 0-1. `build.gradle`에 `spring-boot-starter-data-redis`, `commons-pool2` 의존성만 추가
- [ ] 0-2. `docker-compose.yml` 작성 (redis:7-alpine, maxmemory 정책 포함)
- [ ] 0-3. `docker compose up -d` 후 `redis-cli ping`으로 로컬 기동 확인
