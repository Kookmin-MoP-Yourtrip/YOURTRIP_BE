...
잡은 하나(`build`)이고, 검증은 `./gradlew build` 한 줄이 전부다(그 밖에는 JDK·Gradle 준비와
실패 시 리포트 업로드 스텝뿐이다). `build`가 아래를 모두 포함한다.
...
| **CD 게이트** ([cd.md](cd.md)) | 새 인스턴스의 기동 실패·헬스체크 미통과. 걸리면 교체가 중단돼 **이전 버전이 계속 서비스한다** | 배포마다, 자동 |
| **배포 후 확인** ([profile.md](profile.md) §4-2, [deploy/prod](../../deploy/prod/README.md)) | 프로필 누락, JVM 옵션 미적용 | user-data를 고쳤을 때, 수동 |
...
| CD | JAR → S3 → 배포 | **없음** — OIDC로 실행마다 임시 자격증명을 받는다([cd.md](cd.md)) |
| 런타임 | EC2 부팅 시 설정 주입 | AWS에만 존재, GitHub을 거치지 않음 |

CD가 생기면서 이 표의 두 번째 행이 "AWS 인증 수단만"에서 **"없음"** 으로 바뀌었다. 정적 액세스 키를
GitHub Secrets에 두는 대신 GitHub이 발급한 OIDC 토큰으로 IAM 역할을 맡기 때문이다. 저장되는 비밀이
...
빌드가 실패하면 워크플로가 Gradle의 HTML 테스트 리포트를 **`test-report` 아티팩트로 남긴다.**
실패한 실행의 Actions 페이지 하단 Artifacts에서 내려받아 `index.html`을 열면, 실패한 테스트와
스택트레이스를 로그 스크롤 없이 볼 수 있다. 보관 기간은 7일이다.

성공한 실행에는 아티팩트가 없다(`if: failure()`). 컴파일 단계에서 실패해 테스트까지 가지
못했다면 리포트 자체가 생기지 않으므로, 그때는 로그의 컴파일 오류를 본다.
