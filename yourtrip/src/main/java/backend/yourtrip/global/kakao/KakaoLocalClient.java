...
public class KakaoLocalClient {
    ...
    public KakaoSearchResponse.Document findBestPlace(String placeName, String placeLocation,
        ...
        // 점수가 가장 높은 장소 선택
        return docs.stream()
            .max(Comparator.comparingInt(doc -> score(doc, placeName, placeLocation, placeAddress)))
            .orElse(null);
    }

    private int score(Document doc, String placeName, String placeLocation, String placeAddress) {
        int score = 0;
        ...
        if (placeAddress != null && !placeAddress.isBlank()) {
            ...
            String lowerAIAddr = placeAddress.toLowerCase();
            if (lowerKakaoAddr.contains(lowerAIAddr) || lowerAIAddr.contains(lowerKakaoAddr)) {
                score += 3;
            }
        }

        // 3) 카테고리 그룹이 관광/카페/음식점이면 약간 가산점
