...
public interface MyCourseControllerSpec {
    ...
    @Operation(summary = "나의 코스 단건 조회", description = """
        ...
        - 나의 코스 목록 조회에서 반환된 코스 ID를 경로변수에 넘겨주면 됩니다.
    ...
    @Operation(
        ...
        description = """
            ...
            - 앞서 나의 코스 단건 조회 API에서 제공된 dayId를 경로 변수에 넣어서 해당 일차에 장소를 추가합니다.
    ...
    @Operation(summary = "장소 수정", description = """
        ...
        - 앞서 나의 코스 단건 조회 API에서 제공된 dayId와 일차별 장소 리스트 조회 api에서 제공된 placeId를 경로 변수에 넣어서 해당 일차에 장소를 추가합니다.
    ...
    @Operation(summary = "장소 사진 삭제", description = """
        ...
        - 앞서 나의 코스 단건 조회 API에서 제공된 dayId와 일차별 장소 리스트 조회 api에서 제공된 placeId, placeImageId를 경로 변수에 넣어서 해당 일차에 장소를 추가합니다.
        ...
        - 경로 변수
            ...
            - 일차 ID(dayId): 해당 코스에 존재하는 일차여야 함
            - 장소 ID(placeId): 해당 일차에 존재하는 장소여야 함
            - 장소 이미지 ID(placeImageId): 해당 장소에 존재하는 이미지여야 함. 앞서 장소 사진 추가 혹은 일차별 장소 리스트 조회에서 반환받은 placeImageId를 사용합니다.
        ### ⚠ 예외상황
        - `COURSE_NOT_FOUND(404)`: 코스가 존재하지 않는 경우 (잘못된 courseId가 주어진 경우)
        - `DAY_SCHEDULE_NOT_FOUND(404)`: 해당 코스에 존재하지 않는 일차인 경우 (잘못된 dayId가 주어진 경우)
        - `PLACE_NOT_FOUND(404)`: 해당 일차에 존재하지 않는 장소인 경우 (잘못된 placeId가 주어진 경우)
